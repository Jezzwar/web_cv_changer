...
export default function ReviewStep({ data, onNext, onBack }) {
  ...
  const handleNext = () => onNext({ ...data, adaptedResume: resumeText, name });

  const addedLines = data.missing.length > 0
    ? data.adaptedResume.split('\n').filter(l =>
        data.missing.some(s => l.toLowerCase().includes(s.toLowerCase()))
      )
    : [];
    ...
    <div className="fade-up flex flex-col gap-6">
      <div className="glass-card rounded-2xl p-5 flex flex-col gap-3">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <h3 className="font-display text-xl text-slate-100">Предпросмотр адаптированного резюме</h3>
          {data.missing.length > 0 && (
            <span className="text-xs px-3 py-1 rounded-full bg-violet-500/20 text-violet-300 border border-violet-500/30">
              +{data.missing.length} навыков добавлено
            </span>
          )}
        </div>
        ...
        {data.missing.length > 0 && (
          <div className="flex flex-wrap gap-2 p-3 bg-violet-500/5 border border-violet-500/20 rounded-xl">
            <span className="text-xs text-violet-400 font-medium w-full mb-1">Добавлено AI:</span>
        ...
        <div className="flex flex-col gap-1">
          <label className="text-xs text-slate-500 uppercase tracking-wider">Имя для PDF</label>
          <input
            type="text"
            value={name}
        ...
        <div className="flex flex-col gap-1">
          <label className="text-xs text-slate-500 uppercase tracking-wider">Текст резюме (редактируемый)</label>
          <textarea
            value={resumeText}
            onChange={e => setResumeText(e.target.value)}
            className="w-full h-80 bg-[#0d0d1f] border border-[#2a2a4a] rounded-xl p-3 text-sm text-slate-300 placeholder-slate-600 focus:outline-none focus:border-violet-500 transition-colors font-mono leading-relaxed"
          />
        </div>
      </div>
